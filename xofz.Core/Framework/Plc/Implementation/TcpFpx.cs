namespace xofz.Framework.Plc.Implementation
{
    ...
    internal sealed class TcpFpx : Fpx
    {
        ...
        string Fpx.Location => this.host;

        int Fpx.SecondaryLocation => this.port;
        ...
        private string doOp(string op)
        {
            ...
            lock (this.locker)
            {
                ...
                {
                    if (!client.BeginConnect(h, p, null, null)
                        .AsyncWaitHandle.WaitOne(TimeSpan.FromSeconds(2)))
                    {
                        throw new IOException(
                            "Could not connect to " + h + ":" + p);
                    }
                    ...
                    using (var stream = client.GetStream())
                    {
                        ...
                        var writeBuffer = Encoding.ASCII.GetBytes(
                            op + '\r');
