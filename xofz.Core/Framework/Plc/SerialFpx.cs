// --------------------------------------------------------------------------------------------------------------------
// <copyright file="SerialFpx.cs" company="Care Controls">
...
namespace xofz.Framework.Plc
{
    ...
    public class SerialFpx : Fpx, IDisposable
    {
        public SerialFpx(string portName, int baudRate)
        {
        ...
        public string Location => this.portName;

        public int BaudRate => this.baudRate;
        ...
        {
            var start = ushort.Parse(startAddress.Substring(2)); // string addresses start with DT
        ...
        public void WriteRegister(string address, int value)
        {
            ...
            sb.Append((ushort.Parse(addressValue) + 1)
                .ToString()
                .PadLeft(5, '0'));
